		...
		return layout.Background{}.Layout(gtx,
			func(gtx layout.Context) layout.Dimensions {
				...
				case b.Button.Hovered() || gtx.Focused(b.Button):
					background = HoveredColor(b.Background)
				}
...
func DisabledColor(c color.NRGBA) color.NRGBA {
	return color.NRGBA{R: c.R, G: c.G, B: c.B, A: c.A / 2}
}

func HoveredColor(c color.NRGBA) color.NRGBA {
	return color.NRGBA{
		R: uint8(float32(c.R) * 0.8),
		G: uint8(float32(c.G) * 0.8),
		B: uint8(float32(c.B) * 0.8),
		A: c.A,
	}
}
